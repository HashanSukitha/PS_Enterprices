...
{ 
	...
	j( "#container" ).load( "Reports/Closed_Contracts_Report.php?asAtDate="+asAtDate);
...
<table border="1">
<tr>
  <td>Closed Contract Daily Report</td>
...
<?php
	...
	if(isset($_GET['asAtDate'])!="")
	{
		...
		(SELECT IFNULL(SUM(C.ITM_TOT),0) FROM `contract_details` C ,`contract_header` CC WHERE C.CONTR_NO=CC.CONTR_NO AND C.RNTL_BASIS = 'Daily' AND 
			STR_TO_DATE(CC.CONTR_CLOSE_DATE,'%Y-%m-%d')=STR_TO_DATE('".$_GET['asAtDate']."','%Y-%m-%d') AND CC.CONTR_STS='I')'DAILY',
		...
		(SELECT IFNULL(SUM(C.ITM_TOT),0) FROM `contract_details` C ,`contract_header` CC WHERE C.CONTR_NO=CC.CONTR_NO AND C.RNTL_BASIS = 'Monthly' AND 
			STR_TO_DATE(CC.CONTR_CLOSE_DATE,'%Y-%m-%d')=STR_TO_DATE('".$_GET['asAtDate']."','%Y-%m-%d') AND CC.CONTR_STS='I')'MONTHLY',
			
		
		(SELECT COUNT(*) FROM `contract_header` CC WHERE STR_TO_DATE(CC.CONTR_CLOSE_DATE,'%Y-%m-%d')=STR_TO_DATE('".$_GET['asAtDate']."','%Y-%m-%d') AND CC.CONTR_STS='I')'CONTR_COUNT'
		...
		$sql ="SELECT CC.CONTR_NO'CONTR_NO',
					  ...
					  (SELECT RNTL_BASIS FROM `contract_details` WHERE CONTR_NO=CC.CONTR_NO GROUP BY CONTR_NO)'BASIS',
					   ...
					   CC.CONTR_CLOSE_DATE'CONTR_CLOSE_DATE',
					   (SELECT H_TO FROM contract_details WHERE CONTR_NO=CC.CONTR_NO LIMIT 1)'DUE_DATE',
					   (SELECT ((CC.CONTR_NO-CONTR_NO)+1) FROM contract_header ORDER BY CONTR_NO asc LIMIT 1)'COUNT',
(SELECT SUM(D.CHG) FROM (SELECT (DLY_CHRG*ISSUED_QTY)'CHG',CONTR_NO FROM contract_details) D  WHERE D.CONTR_NO=CC.CONTR_NO )'DLY_CHRG',
(SELECT SUM(D.CHG) FROM (SELECT (MNLY_CHG*ISSUED_QTY)'CHG',CONTR_NO FROM contract_details) D WHERE D.CONTR_NO=CC.CONTR_NO )'MNLY_CHG'
		 		FROM `contract_header` CC WHERE	STR_TO_DATE(CC.CONTR_CLOSE_DATE,'%Y-%m-%d')=STR_TO_DATE('".$_GET['asAtDate']."','%Y-%m-%d') AND CC.CONTR_STS='I'";
